# Gempper — Flipper Zero FAP

*By Kasen Sansonetti & the Wetware Labs team. [WetwareOfficial.com](https://WetwareOfficial.com)*

## Identity

This is **Gempper**, a Flipper Zero FAP (Flipper Application Package) that turns the Flipper into:
1. A one-handed HID remote for controlling Gemini CLI (numbered approvals + voice trigger + Enter) — USB or Bluetooth.
2. An offline Gemini CLI manual with folder navigation and quiz mode, browsable on the Flipper's 128x64 display.

Two builds from one codebase:
- **gemini_remote_usb** — USB HID, works on stock firmware (App Catalog eligible).
- **gemini_remote_ble** (named "Gempper") — BLE HID, works on Momentum/Unleashed firmware (wireless). Primary build.
...
- Entry point signatures: `int32_t gemini_remote_usb_app(void* p)` and `int32_t gemini_remote_ble_app(void* p)`, both returning 0.
...
├── gemini_remote.c                    # main app (~1900 lines), all modes, #ifdef transport
├── gemini_remote.png                  # 10x10 1-bit app icon
...
│   └── prd.md
└── dist/                              # build output
    ├── gemini_remote_usb.fap          # stock firmware build
    └── gemini_remote_ble.fap          # Momentum/Unleashed build (primary)
```
...
App(
    appid="gemini_remote_usb",
    name="Gemini Remote USB",
    apptype=FlipperAppType.EXTERNAL,
    entry_point="gemini_remote_usb_app",
    requires=["gui"],
    stack_size=2 * 1024,
    fap_category="Bluetooth",
    fap_icon="gemini_remote.png",
...
App(
    appid="gemini_remote_ble",
    name="Gempper",
    apptype=FlipperAppType.EXTERNAL,
    entry_point="gemini_remote_ble_app",
    requires=["gui", "bt"],
    stack_size=2 * 1024,
    fap_category="Bluetooth",
    fap_icon="gemini_remote.png",
...
- "Gempper Remote" title, "LABS" right-aligned, "Flipper's Gemini pal" tagline
- Auto-advances after 3 seconds or on any key press

### Home Screen (portrait 64x128)

- "Gemini Remote" title with D-pad pixel art illustration
...
- **Getting Started** (4): Installing Gemini CLI, First Launch, System Requirements, Authentication
...
| GitHub repo | https://github.com/Wet-wr-Labs/gempper |
