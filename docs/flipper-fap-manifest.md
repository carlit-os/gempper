...
```bash
mkdir gemini_remote && cd gemini_remote
ufbt update --channel=dev
ufbt create APPID=gemini_remote
```

This generates:
- `application.fam` — manifest
- `gemini_remote.c` — main source
- `gemini_remote.png` — 10x10 1-bit icon
...
```python
App(
    appid="gemini_remote",              # unique ID, used in build commands
    name="Gemini Remote",              # display name in Flipper menus
    apptype=FlipperAppType.EXTERNAL,   # runs from SD card
    entry_point="gemini_remote_app",   # int32_t function_name(void* p)
...
|-------|----------|-------------|
| `appid` | Yes | Unique identifier. Lowercase, underscores. Used in build: `./fbt fap_gemini_remote` |
...
| `./fbt fap_gemini_remote` | Build specific app (full firmware tree) |
| `./fbt launch APPSRC=applications_user/gemini_remote` | Build & upload (full tree) |

## FAP Assets

- Images must be 1-bit (monochrome) PNGs.
- Place in the directory specified by `fap_icon_assets`.
- Access in code via: `#include "gemini_remote_icons.h"`
