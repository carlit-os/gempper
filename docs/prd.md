# Gemini Remote — Product Requirements Document

## 1. Product Overview

**Product name:** Gemini Remote
**Platform:** Flipper Zero (FAP app)
**Goal:**
Turn the Flipper Zero into:
1. A **one-handed remote** for controlling Gemini CLI (numbered approvals + voice input).
2. An **offline Gemini CLI manual**, browsable entirely on the Flipper.

The same app should let the user:
- Drive Gemini CLI using 1/2/3 + Enter + Voice from the D-pad.
- Read Gemini CLI docs, shortcuts, and best-practice "playbooks" when away from their Mac.
...
1. **Remote agent control while watching the terminal:**
   - User is at their Mac, Gemini CLI is running.
...
- AI-heavy developers using Gemini CLI as their main coding agent.
...
### 4.2 Gemini Manual Mode

An offline, structured manual fully browsable on Flipper.

**Sections:**
1. Getting Started — What Gemini CLI is, basic concepts
...
- Plain text files on SD card: `/ext/apps_data/gemini_remote/manual/01_*.txt` through `05_*.txt`
...
- External tooling (Mac script) can regenerate `.txt` content from Gemini CLI docs.
...
1. As a Gemini CLI user, I want to press Left/Up/Right on my Flipper to send 1/2/3 to Gemini CLI so I can answer approval prompts without touching my keyboard.
...
6. As a user, I want the app to show a warning when Bluetooth HID is not connected so I understand why presses don't reach Gemini CLI.

### Manual / Docs

7. As a Gemini CLI user, I want to open a "Gemini Manual" on my Flipper so I can read how Gemini CLI works offline.
