# Gemini Remote — User Manual

*By Kasen Sansonetti & the Wetware Labs team. [WetwareOfficial.com](https://WetwareOfficial.com)*

## What Is This?

Gemini Remote turns your Flipper Zero into a one-handed remote control for Gemini CLI. Use the D-pad to approve prompts (1/2/3), hit Enter, or trigger voice dictation — without touching your keyboard.
...
It also includes a built-in offline Gemini CLI manual you can read anywhere.
...
- Mac (or any computer running Gemini CLI in a terminal)

### Which version should I use?

| Version | File | Firmware | Connection |
|---------|------|----------|------------|
| USB | `gemini_remote_usb.fap` | Any (stock, Momentum, Unleashed) | USB-C cable |
| BLE | `gemini_remote_ble.fap` | Momentum or Unleashed only | Bluetooth wireless |
...
ufbt launch APPSRC=gemini_remote_usb               # upload + run USB version
```

The Flipper must be connected via USB for `ufbt launch` to work.

Both `.fap` files are output to `dist/`:
- `dist/gemini_remote_usb.fap`
- `dist/gemini_remote_ble.fap`
...
5. The app will appear on Flipper under: **Apps > Bluetooth > Gemini Remote USB** (or BLE)
...
2. Copy the generated files to the Flipper SD card:
   ```bash
   mkdir -p /Volumes/FLIPPER/apps_data/gemini_remote/manual/
   cp manual_output/*.txt /Volumes/FLIPPER/apps_data/gemini_remote/manual/
   ```
...
## Part 2: Using Gemini Remote

### Launching

On your Flipper: **Apps > Bluetooth > Gemini Remote**
...
| Down or Left | Enter Gemini Manual mode |
| Back | Exit the app |
...
**USB Setup:**
1. Open Gemini CLI in your terminal on your Mac.
2. Plug the Flipper into the same Mac via USB-C.
3. Launch Gemini Remote USB on the Flipper.
4. Press OK to enter Remote Control mode.

**BLE Setup (Momentum/Unleashed only):**
1. Open Gemini CLI in your terminal on your Mac.
2. Launch Gemini Remote BLE on the Flipper.
...
| Flipper Button | What It Sends | Gemini CLI Meaning |
|----------------|---------------|---------------------|
| Left | `1` | Yes / Approve |
| Up | `2` | No / Decline |
...
**Typical workflow:**
1. Gemini CLI shows a prompt with options 1/2/3.
...
4. If you use a different shortcut, you'll need to modify the `HID_KEYBOARD_F5` constant in `gemini_remote.c` and rebuild.
...
To publish Gemini Remote on the official Flipper App Catalog:
...
4. **10x10 1-bit PNG icon** (already included: `gemini_remote.png`).
...
- Orientation flip
- Offline Gemini CLI manual with 5 chapters
...
- Open qFlipper.
- Launch Gemini Remote on the Flipper.
...
git checkout -b yourusername/gemini_remote_0.1
```

**4. Create your manifest:**

Create `applications/Bluetooth/gemini_remote/manifest.yml`:
...
short_description: "One-handed remote for Gemini CLI + offline manual"
...
```bash
python3 tools/bundle.py --nolint applications/Bluetooth/gemini_remote/manifest.yml bundle.zip
```
...
|----------|---------------|-----|
| "Down button toggles macOS/Gemini voice input" | Sends F5 keycode | Works only if macOS dictation is configured to F5. No runtime configurability — changing the shortcut key requires editing source code and rebuilding. |

The F5 key is hardcoded. If the user's dictation shortcut is different (e.g., fn-fn, or a custom key), they must change `HID_KEYBOARD_F5` in `gemini_remote.c` and rebuild.
...
- Gemini Manual with 7 categories and 29 sections covering all major Gemini CLI features
- Manual scrolling (Up/Down) and chapter switching (Left/Right)
- SD card manual loading from `/ext/apps_data/gemini_remote/manual/`
...
*Gemini Remote is by Kasen Sansonetti & the Wetware Labs team. [WetwareOfficial.com](https://WetwareOfficial.com)*
