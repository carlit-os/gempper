...
static void draw_splash(Canvas* canvas) {
    ...
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str_aligned(canvas, 64, 58, AlignCenter, AlignCenter, "Flipper's Gemini pal");
}
...
static void draw_tour(Canvas* canvas, GeminiRemoteState* state) {
    ...
    if(state->tour_page == 0) {
        ...
        canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignCenter, "Learn Gempper basics");
...
static void draw_quiz_multichoice(Canvas* canvas, GeminiRemoteState* state, const QuizCard* card) {
...
static void handle_tour_input(GeminiRemoteState* state, InputEvent* event, ViewPort* vp) {
...
static bool handle_home_input(GeminiRemoteState* state, InputEvent* event, ViewPort* vp) {
...
static bool handle_remote_input(GeminiRemoteState* state, InputEvent* event, ViewPort* view_port) {
...
static void handle_manual_categories(GeminiRemoteState* state, InputEvent* event, ViewPort* vp) {
    switch(event->key) {
    case InputKeyUp:
...
static void handle_manual_sections(GeminiRemoteState* state, InputEvent* event) {
    const ManualCategory* cat = &categories[state->cat_index];
...
static bool handle_manual_input(GeminiRemoteState* state, InputEvent* event, ViewPort* vp) {
...
static bool handle_settings_input(GeminiRemoteState* state, InputEvent* event, ViewPort* vp) {
    if(event->type != InputTypeShort) return true;
...
static bool handle_macros_input(GeminiRemoteState* state, InputEvent* event, ViewPort* vp) {
    if(event->type != InputTypeShort) return true;
...
int32_t gemini_remote_usb_app(void* p) {
    return gemini_remote_main(p);
}

int32_t gemini_remote_ble_app(void* p) {
    return gemini_remote_main(p);
}
